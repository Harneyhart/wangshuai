...
// delete homework by id (hard delete with cascade)
export async function deleteHomeworkById(homeworkId: string) {
  try {
    return await db.transaction(async (tx) => {
      // 1. 获取该作业的所有提交
      const homeworkSubmissions = await tx.query.submissions.findMany({
        where: (table, { eq }) => eq(table.homeworkId, homeworkId),
      });
      ...
      // 4. 删除作业记录
      return await tx.delete(schema.homeworks)
        .where(eq(schema.homeworks.id, homeworkId))
        .returning();
    });
  } catch (error) {
    console.error('deleteHomeworkById error:', error);
    throw new Error('deleteHomeworkById error');
  }
}
// update homework
...
export async function deleteAttachmentById(id: string) {
  try {
    ...
    await Promise.all([
      ...
      db
        .delete(schema.submissionsToAttachments)
        .where(eq(schema.submissionsToAttachments.attachmentId, id)),
      // 如果还有其他关联表，也需要在这里添加
    ]);
