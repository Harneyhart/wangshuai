...
import { getSubmissionsForCurrentTeacher, getAttachmentsByCoursePlanId, getClassesForCurrentTeacher } from '@/lib/course/actions';
...
const TurnHomeworkManager: React.FC<TurnHomeworkManagerProps> = ({ courseId }) => {
    ...
    useEffect(() => {
        const loadHomeworkData = async () => {
            try {
                    ...
                    if (!homeworkMap.has(homeworkId)) {
                        homeworkMap.set(homeworkId, {
                            ...
                            createdAt: submission.homework?.createdAt ? 
                                ...
                                : undefined,
                            courseId: submission.homework?.plan?.course?.id
                        });
                    }
                ...
                const filtered = teacherClassIds.length > 0
                    ? homeworkArray.filter(item => teacherClassIds.includes(item.plan?.class?.id))
                    : [];
                setData(filtered);
                
                if (filtered.length > 0) {
                    message.success(`成功加载 ${filtered.length} 个作业`);
                }
            } catch (error) {
    ...
    const refreshSubmissions = async () => {
        try {
            ...
            const filtered = teacherClassIds.length > 0
                ...
                : [];
            setData(filtered);
            message.success(`刷新成功，共 ${filtered.length} 个作业`);
        } catch (error) {
    ...
    useEffect(() => {
        const fetchTeacherClasses = async () => {
            ...
            if (Array.isArray(classes)) {
                setTeacherClassIds(classes.map(cls => cls.id));
            }
        };
        fetchTeacherClasses();
    }, []);
...
export default TurnHomeworkManager;